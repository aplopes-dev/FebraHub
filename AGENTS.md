...
- **Cliente diz que é integração da MAQUININHA (POS físico)** → não é gateway e-commerce. Caminhos possíveis: (a) **Stone Connect 2.0** (API TEF Software-de-Gestão↔maquininha, cria pedido de venda na maquininha; doc `ajuda.stone.com.br/connect-20`), (b) SDK Android (POS Positivo/próprio app na maquininha), (c) TapTon/Tap-to-Pay. StoneProvider atual NÃO serve p/ maquininha. Aguardando: confirmar produto exato + chave/credencial válida + modelo de maquininha antes de implementar.
