...
export type CancelStoreSignaturePackageRequestInput = {
  storeId: string;
  requestId: string;
};

@Injectable()
export class CancelStoreSignaturePackageRequestUseCase implements IUseCase<
  CancelStoreSignaturePackageRequestInput,
...
> {
  ...
  async execute(
    input: CancelStoreSignaturePackageRequestInput,
  ): Promise<SignaturePackageRequestDto> {
    const store = await this.storeRepository.findById(input.storeId);
    if (!store) {
      throw new StoreNotFoundError(
        CancelStoreSignaturePackageRequestUseCase.name,
        input.storeId,
      );
    }
    ...
    return this.provisioning.cancel(input.storeId, input.requestId);
  }
}
