...
export type LiberateStoreSignaturePackageRequestInput = {
  storeId: string;
  requestId: string;
};

@Injectable()
export class LiberateStoreSignaturePackageRequestUseCase implements IUseCase<
  LiberateStoreSignaturePackageRequestInput,
...
> {
  ...
  async execute(
    input: LiberateStoreSignaturePackageRequestInput,
  ): Promise<SignaturePackageRequestDto> {
    const store = await this.storeRepository.findById(input.storeId);
    if (!store) {
      throw new StoreNotFoundError(
        LiberateStoreSignaturePackageRequestUseCase.name,
        input.storeId,
      );
    }
    ...
    return this.provisioning.liberate(input.storeId, input.requestId);
  }
}
