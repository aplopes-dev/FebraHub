...
import { UnblockStoreUseCase } from './unblock-store.use-case';
import { createNoopStoreEventsPublisher } from '../../../tests/noop-store-events.publisher';
import { BlockStoreUseCase } from '../block-store/block-store.use-case';
...
describe('UnblockStoreUseCase', () => {
  let useCase: UnblockStoreUseCase;
  let blockUseCase: BlockStoreUseCase;
  ...
  beforeEach(() => {
    ...
    useCase = new UnblockStoreUseCase(
      storeRepo,
      detailRepo,
      createNoopStoreEventsPublisher(),
      createPassThroughUnitOfWork(),
    );
    blockUseCase = new BlockStoreUseCase(
      storeRepo,
      detailRepo,
      createNoopStoreEventsPublisher(),
      createPassThroughUnitOfWork(),
  ...
  it('should unblock store', async () => {
    const dto = await buildCreateStoreFixture(planRepo, subscriptionRepo, {
      slug: 'maria-doces-unblock',
    });
    const created = await createStore.execute(dto);

    await blockUseCase.execute({ id: created.store.id, actor: TEST_ACTOR });
    const unblocked = await useCase.execute({
      id: created.store.id,
      actor: TEST_ACTOR,
    });
    expect(unblocked.status).toBe('PRODUCTION');
  });

  it('should be idempotent when not blocked', async () => {
    const dto = await buildCreateStoreFixture(planRepo, subscriptionRepo, {
      tradeName: 'Maria Doces 2',
      slug: 'maria-doces-unblock-2',
    });
    const created = await createStore.execute(dto);
