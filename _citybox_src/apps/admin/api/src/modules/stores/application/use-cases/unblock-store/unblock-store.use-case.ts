...
@Injectable()
export class UnblockStoreUseCase implements IUseCase<BlockStoreDto, Store> {
  constructor(
  ...
  async execute({ id, actor }: BlockStoreDto): Promise<Store> {
    ...
    if (!store) {
      throw new StoreNotFoundError(UnblockStoreUseCase.name, id);
    }
