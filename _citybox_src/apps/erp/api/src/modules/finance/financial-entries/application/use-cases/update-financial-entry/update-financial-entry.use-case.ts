...
> {
  ...
  async execute(input: UpdateFinancialEntryDto): Promise<FinancialEntry> {
    ...
    // `entry.update()` já rejeita (SaleOrderLinkedEntryForbiddenError) se o
    // lançamento estiver vinculado a um pedido de venda.
    return this.financialEntryRepository.save(
      entry.update({
