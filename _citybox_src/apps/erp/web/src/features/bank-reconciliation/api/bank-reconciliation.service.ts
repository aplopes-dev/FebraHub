...
export async function listBankStatementsApi(
  ...
  const res = await comercioFetch<BankStatementListResponseDto>(
...
export async function findBankStatementByIdApi(id: string) {
  const res = await comercioFetch<{ data: BankStatementDto }>(
...
export async function deleteBankStatementApi(id: string): Promise<void> {
  await comercioFetch<void>(`/v1/bank-statements/${id}`, { method: "DELETE" });
}
...
export async function importBankStatementApi(
  ...
  const res = await comercioUpload<ImportBankStatementResponseDto>(
    "/v1/bank-statements",
    formData,
  );
...
export async function previewBankStatementApi(
  ...
  const formData = new FormData();
  formData.append("file", file);
  const res = await comercioUpload<{ data: PreviewBankStatementResult }>(
...
export async function listStatementTransactionsApi(
  ...
  const res = await comercioFetch<BankStatementTransactionListResponseDto>(
...
export async function suggestMatchesApi(
  ...
  const res = await comercioFetch<MatchSuggestionResponseDto>(
...
export async function searchEligibleEntriesApi(
  ...
  const res = await comercioFetch<EligibleEntrySearchResponseDto>(
...
export async function createEntryFromTransactionApi(
  ...
  await comercioFetch<CreateEntryFromTransactionResponseDto>(
...
export function bankStatementFileUrl(bankStatementId: string): string {
  ...
  return `/api/proxy/comercio/v1/bank-statements/${bankStatementId}/file${query ? `?${query}` : ""}`;
}
