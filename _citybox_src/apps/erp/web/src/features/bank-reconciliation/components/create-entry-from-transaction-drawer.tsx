...
import { Autocomplete, Button, Drawer, Input, ScrollArea, Select, Stack } from "@citybox/mui";
...
function CreateEntryFromTransactionDrawerBody({
  ...
  const [partyValue, setPartyValue] = useState<string | null>(null);
  ...
  const selectedPartyOption =
    partyOptions.find((option) => option.value === partyValue) ?? null;
    ...
    <Stack spacing={2} sx={{ height: "100%", minHeight: 0 }}>
        ...
        <Box sx={{ display: "grid", gap: 3 }}>
          ...
          <Box>
            ...
            <Autocomplete
              ...
              onChange={(_, option: PartyComboboxOption | null) =>
                setPartyValue(option?.value ?? null)
              }
      ...
      <Stack direction="row" spacing={1} sx={{ justifyContent: "flex-end" }}>
        ...
        <Button
          ...
          loading={isSaving}
          onClick={() => {
            const parsedParty = partyValue ? parsePartyValue(partyValue) : null;
            const partyName = selectedPartyOption?.label ?? "";
            onConfirm({
              description,
              partyName,
              customerId: parsedParty?.kind === "customer" ? parsedParty.id : null,
              supplierId: parsedParty?.kind === "supplier" ? parsedParty.id : null,
              categoryName: "",
              note,
              bankAccountId,
              chartOfAccountId,
