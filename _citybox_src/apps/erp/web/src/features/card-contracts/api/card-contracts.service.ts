...
export async function listCardContracts(
  ...
  const response = await comercioFetch<CardContractListResponseDto>(
...
export async function getCardContractById(
  ...
  const response = await comercioFetch<CardContractResponseDto>(
...
export async function deleteCardContract(id: string): Promise<void> {
  await comercioFetch<void>(`/v1/card-contracts/${id}`, { method: "DELETE" });
}
...
export async function listPaymentMethods(
  ...
  const response = await comercioFetch<CardPaymentMethodListResponseDto>(
...
export async function createPaymentMethod(
  ...
  const response = await comercioFetch<CardPaymentMethodResponseDto>(
    `/v1/card-contracts/${contractId}/payment-methods`,
    { method: "POST", body: JSON.stringify(payload) },
  );
