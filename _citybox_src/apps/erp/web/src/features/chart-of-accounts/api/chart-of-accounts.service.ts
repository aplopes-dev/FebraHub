...
export async function listChartOfAccounts(
  ...
  const response = await comercioFetch<ChartOfAccountListResponseDto>(
...
export async function getChartOfAccountByIdApi(
  ...
  const response = await comercioFetch<ChartOfAccountResponseDto>(
...
export async function deleteChartOfAccount(id: string): Promise<void> {
  await comercioFetch<void>(`/v1/chart-of-accounts/${id}`, {
    method: "DELETE",
  });
}
