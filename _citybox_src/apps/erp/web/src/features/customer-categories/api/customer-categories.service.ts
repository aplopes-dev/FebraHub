...
export async function listCustomerCategories(
  ...
  const response = await comercioFetch<CustomerCategoryListResponseDto>(
...
export async function createCustomerCategory(
  ...
  const response = await comercioFetch<CustomerCategoryResponseDto>(
...
export async function deleteCustomerCategory(id: string): Promise<void> {
  await comercioFetch<void>(`/v1/customer-categories/${id}`, {
    method: "DELETE",
  });
}
