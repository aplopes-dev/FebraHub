...
/** Detalhe completo para a tela de edição (spec erp/029) — molde de
 * `useSupplierQuery`/`useCustomerQuery`, mas mapeado direto para
 * `CustomerFormValues` (evita o mapper enxuto de listagem, que descarta
 * documento/endereços). */
export function useCustomerFormValuesQuery(id: string) {
  const { scope, ready } = useCatalogScope();

  return useQuery({
    queryKey: [...customerKeys.detail(scope, id), "form-values"],
    queryFn: () => getCustomerFormValuesById(id),
    enabled: ready && Boolean(id),
    retry: false,
  });
}
