...
/**
 * Aba "Emitido" — agora pela **erp-api**, não pela fiscal-api direto.
 *
 ...
 * a ser a `erp-api`, a partir da organização ativa do tenant. Um `companyId`
 ...
 * Emitente) — essas ainda não migraram para a erp-api e seguem no padrão
...
/** `GET /v1/fiscal/documents` (erp-api) — busca/paginação backend-driven. */
export async function listIssuedFiscalDocumentsApi(
  ...
  const res = await comercioFetch<FiscalDocumentListResponseDto>(
...
export async function getIssuedFiscalDocumentsSummaryApi(
  ...
  const res = await comercioFetch<FiscalDocumentSummaryResponseDto>(
...
export async function findFiscalCompanyByCnpjApi(
  cnpj: string,
): Promise<{ id: string; defaultEnvironment: "HOMOLOGATION" | "PRODUCTION" } | null> {
  ...
  return company
    ? { id: company.id, defaultEnvironment: company.defaultEnvironment }
    : null;
}
