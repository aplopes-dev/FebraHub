...
export async function listFinancialGroups(
  ...
  const response = await comercioFetch<FinancialGroupListResponseDto>(
...
export async function getFinancialGroupByIdApi(
  ...
  const response = await comercioFetch<FinancialGroupResponseDto>(
...
export async function deleteFinancialGroup(id: string): Promise<void> {
  await comercioFetch<void>(`/v1/financial-groups/${id}`, {
    method: "DELETE",
  });
}
