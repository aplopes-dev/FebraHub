...
export async function listPriceLists(
  ...
  const response = await comercioFetch<PriceListListResponseDto>(
...
export async function getPriceListById(id: string): Promise<PriceList | null> {
  try {
    const response = await comercioFetch<PriceListResponseDto>(
      `/v1/price-lists/${id}`,
    );
...
export async function deletePriceList(id: string): Promise<void> {
  await comercioFetch<void>(`/v1/price-lists/${id}`, { method: "DELETE" });
}

export async function reorderPriceLists(
  orderedIds: string[],
): Promise<PriceList[]> {
  const response = await comercioFetch<{ data: PriceListListResponseDto["data"] }>(
...
export async function getPriceListItems(
  ...
  const response = await comercioFetch<PriceListItemsResponseDto>(
