...
Roteiro manual ponta a ponta do que **já persiste** no ERP Comércio.
...
**Ainda só UI (não falhar se sumir no reload):** Disponibilidade/canais, Adicionais, Sugestões; insumos da ficha técnica; importação XLSX; duplicar variação.
...
- [ ] Migration OK: `pnpm --filter @citybox/erp-api db:migrate:status` → *up to date*
...
### 3.5 Disponibilidade / Adicionais / Sugestões (só UI)

- [ ] Pode preencher algo nestas seções **sem** esperar persistência
- [ ] Não usar isso como critério de aceite de gravação
...
4. Em DevTools → Network: request falha (`/api/proxy/comercio/v1/...`), status e body
