...
export async function listProducts(
  ...
  const [response, categories] = await Promise.all([
    comercioFetch<ProductListResponseDto>(
...
export async function getProductById(
  ...
  const response = await comercioFetch<ProductResponseDto>(
    `/v1/products/${id}`,
  );
...
export async function bulkDeleteProducts(
  ...
  const response = await comercioFetch<{ data: { affected: number } }>(
...
export async function downloadProductImportTemplateFromApi(): Promise<void> {
  ...
  const response = await fetchWithSession(
    `${COMERCIO_PROXY}/v1/products/import/template`,
    { headers },
  );

  if (!response.ok) {
    throw new ComercioApiError(
      response.status,
...
export async function importProducts(file: File): Promise<ProductImportResult> {
  ...
  const response = await comercioUpload<{ data: ProductImportResult }>(
    "/v1/products/import",
    formData,
  );
  return response.data;
...
export async function listProductCategories(
  ...
  const response = await comercioFetch<{ data: ProductCategoryDto[] }>(
...
export async function listUnitsOfMeasure(
  ...
  const response = await comercioFetch<{ data: UnitOfMeasureDto[] }>(
...
export async function uploadProductImage(
  ...
  const formData = new FormData();
  formData.append("file", file);
  const response = await comercioUpload<ProductResponseDto>(
