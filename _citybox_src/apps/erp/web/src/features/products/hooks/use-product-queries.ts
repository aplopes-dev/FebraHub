...
import { listProductAddons } from "@/features/products/api/product-addons.service";
...
export function useProductAddonsQuery() {
  const { scope, ready } = useCatalogScope();

  return useQuery({
    queryKey: productKeys.addons(scope),
    queryFn: listProductAddons,
    enabled: ready,
    staleTime: 60_000,
  });
}
