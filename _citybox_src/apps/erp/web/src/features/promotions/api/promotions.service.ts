...
export async function getPromotionByIdApi(
  ...
  try {
    return await comercioFetch<PromotionDto>(`/v1/promotions/${id}`);
  } catch {
    return null;
  }
}
...
export async function createPromotionApi(
  ...
  return comercioFetch<PromotionDto>("/v1/promotions", {
    method: "POST",
...
export async function updatePromotionApi(
  ...
  return comercioFetch<PromotionDto>(`/v1/promotions/${id}`, {
    method: "PUT",
    body: JSON.stringify(input),
  });
}
...
export async function deletePromotionApi(id: string): Promise<void> {
  await comercioFetch<void>(`/v1/promotions/${id}`, { method: "DELETE" });
}

export async function restorePromotionApi(id: string): Promise<PromotionDto> {
  return comercioFetch<PromotionDto>(`/v1/promotions/${id}/restore`, {
    method: "POST",
  });
}
...
export async function previewPromotionApi(input: {
  ...
  return comercioFetch<{ discountCents: number }>("/v1/promotions/preview", {
    method: "POST",
    body: JSON.stringify(input),
  });
}
