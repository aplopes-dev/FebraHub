...
- **Alterar status** — escolhe o novo status (com uma bolinha de cor ao lado
  de cada opção).
- **Gerar nota fiscal** — opções de NFe e NFCe (ainda indisponíveis nesse
  submenu; a emissão de NF-e acontece na tela própria, em Vendas › Fiscal ›
  NF-e). No mesmo submenu, **Baixar XML (NF-e)** e **Baixar DANFE** baixam o
  arquivo da nota já emitida para esse pedido — só ficam disponíveis quando a
  NF-e foi **autorizada** pelo órgão; sem emissão ou com nota rejeitada, o
  item fica desabilitado explicando o motivo.
...
⚠️ A lista de formas de pagamento vem do cadastro em Configurações → Formas
de pagamento — se uma forma não aparecer aqui, cadastre-a lá primeiro.
