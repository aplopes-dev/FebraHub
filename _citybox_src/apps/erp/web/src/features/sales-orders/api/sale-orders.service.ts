...
export async function listSaleOrdersApi(
  ...
  const response = await comercioFetch<SaleOrderListResponseDto>(
...
export async function getSaleOrderByIdApi(id: string): Promise<SaleOrder> {
  const response = await comercioFetch<SaleOrderDetailResponseDto>(
    `/v1/sale-orders/${id}`,
  );
...
export async function createSaleOrderApi(
  ...
  const response = await comercioFetch<SaleOrderSingleResponseDto>(
    "/v1/sale-orders",
    {
      method: "POST",
...
export async function deleteSaleOrderApi(id: string): Promise<void> {
  await comercioFetch<void>(`/v1/sale-orders/${id}`, { method: "DELETE" });
}
