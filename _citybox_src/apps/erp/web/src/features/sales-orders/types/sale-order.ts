...
/** Vínculo NF-e do pedido (spec erp/029) — habilita "Baixar XML"/"Baixar
 * DANFE" no menu de ações; `null` quando o pedido não tem NF-e emitida. */
export type SaleOrderNfeIssuance = {
  id: string;
  status: string;
...
export type SaleOrder = {
  ...
  /** `undefined` = pedido gerado antes desta feature/fora da listagem
   * paginada; `null` = sem NF-e emitida. */
  nfeIssuance?: SaleOrderNfeIssuance | null;
