...
export async function listServiceOrderStatusesApi(): Promise<
  ServiceOrderStatus[]
> {
  const res = await comercioFetch<{ data: ServiceOrderStatusHttpDto[] }>(
  ...
  return res.data.map((row) => ({
    ...
    variant: row.variant as ServiceOrderStatusVariant,
...
/**
 * Bugfix (2026-08-20, achado em teste manual): `active`/`variant` já eram
 * editados no form do gerenciador de status, mas o `POST`/`PUT` não os
 * aceitavam — o backend descartava os dois em silêncio.
 */
export async function createServiceOrderStatusApi(input: {
  ...
  const row = await comercioFetch<ServiceOrderStatusHttpDto>(
    "/v1/service-order-statuses",
...
export async function deleteServiceOrderStatusApi(id: string): Promise<void> {
  await comercioFetch<void>(`/v1/service-order-statuses/${id}`, {
    method: "DELETE",
  });
}
...
export async function listServiceOrdersApi(
  ...
  const response = await comercioFetch<ListResponse>(
...
export async function getServiceOrderByIdApi(
  ...
  const dto = await comercioFetch<ServiceOrderHttpDto>(
...
export async function createServiceOrderApi(
  ...
  const dto = await comercioFetch<ServiceOrderHttpDto>("/v1/service-orders", {
    method: "POST",
...
export type GenerateSaleFromServiceOrderPayment = {
  amountCents: number;
  methodId: string;
...
/**
 * Bugfix (2026-08-20, achado em teste manual): antes não mandava
 * `payments`, e sem pagamentos o motor de recebíveis (`maybeCreateReceivable`
 * na erp-api) nunca cria o `FinancialEntry` de fornecimento de serviço — a
 * venda fechava "invisível" para o financeiro.
 */
export async function generateSaleFromServiceOrderApi({
  id,
  payments,
}: {
  id: string;
  payments: GenerateSaleFromServiceOrderPayment[];
}): Promise<{
  saleId: string;
  saleNumber: number;
}> {
  const sale = await comercioFetch<{ id: string; number: number }>(
