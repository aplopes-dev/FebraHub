...
export async function listInventoriesApi(
  ...
  const response = await comercioFetch<InventoryListResponseDto>(
...
export async function getInventoryByIdApi(id: string): Promise<Inventory> {
  const response = await comercioFetch<InventoryDetailResponseDto>(
    `/v1/inventories/${id}`,
  );
...
export async function createInventoryApi(
  ...
  const response = await comercioFetch<InventoryCreatedResponseDto>(
