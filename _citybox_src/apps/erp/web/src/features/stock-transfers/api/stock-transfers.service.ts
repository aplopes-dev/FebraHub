...
export async function listStockTransfersApi(
  ...
  const response = await comercioFetch<StockTransferListResponseDto>(
...
export async function createStockTransferApi(
  ...
  await comercioFetch<StockTransferCreatedResponseDto>(
    `/v1/stock-transfers`,
    {
      method: "POST",
...
export async function cancelStockTransferApi(id: string): Promise<void> {
  await comercioFetch<StockTransferCancelledResponseDto>(
