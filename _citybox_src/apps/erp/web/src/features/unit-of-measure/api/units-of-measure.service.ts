...
export async function listUnitsOfMeasurePaginated(
  ...
  const response = await comercioFetch<UnitOfMeasureListResponseDto>(
...
export async function createUnitOfMeasure(
  ...
  const response = await comercioFetch<UnitOfMeasureResponseDto>(
    "/v1/units-of-measure",
...
export async function deleteUnitOfMeasure(id: string): Promise<void> {
  await comercioFetch<void>(`/v1/units-of-measure/${id}`, {
    method: "DELETE",
  });
}
