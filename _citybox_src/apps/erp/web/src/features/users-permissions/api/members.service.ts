...
export async function listMembers(
  ...
  if (params.tab === "active") {
    const [activeRes, allRes] = await Promise.all([
      comercioFetch<MemberListResponseDto>(
    ...
    // Filtro opcional por perfil — a API não tem esse query param.
    let data = activeRes.data.map(toPlatformUser);
    if (params.profileId !== "all") {
      data = data.filter((user) => user.profileId === params.profileId);
    }

    return {
      data,
      meta: {
        total: params.profileId === "all" ? activeRes.meta.total : data.length,
        page: activeRes.meta.page,
        perPage: activeRes.meta.perPage,
        totalPages:
          params.profileId === "all"
            ? activeRes.meta.totalPages
            : Math.max(1, Math.ceil(data.length / params.perPage)),
      },
      tabCounts: {
  ...
  const allRes = await comercioFetch<MemberListResponseDto>(
    `/v1/members?${buildListQuery({
      search: params.search,
      page: 1,
  ...
  if (params.profileId !== "all") {
    inactive = inactive.filter((user) => user.profileId === params.profileId);
  }
...
export async function getMemberById(id: string): Promise<PlatformUser> {
  const response = await comercioFetch<MemberListResponseDto>(
    `/v1/members?page=1&perPage=${MAX_PER_PAGE}`,
  );
...
export async function createMember(
  ...
  const response = await comercioFetch<CreateMemberResponseDto>("/v1/members", {
    method: "POST",
...
export async function updateMember(
  ...
  const response = await comercioFetch<MemberResponseDto>(`/v1/members/${id}`, {
    method: "PUT",
    body: JSON.stringify(payload),
  });
...
export async function resetMemberPassword(
  ...
  const response = await comercioFetch<ResetMemberPasswordResponseDto>(
...
export async function setMemberPdvPin(
  ...
  const response = await comercioFetch<MemberResponseDto>(
