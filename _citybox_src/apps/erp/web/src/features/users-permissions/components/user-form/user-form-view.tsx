...
import { UserBranchesSection } from "@/features/users-permissions/components/user-form/user-branches-section";
...
import {
  UserPdvSection,
  type UserPdvStatus,
} from "@/features/users-permissions/components/user-form/user-pdv-section";
...
type UserFormViewProps = {
  ...
  backHref: string;
  /** membershipId na edição (reset de senha / PIN PDV). */
  memberId?: string;
  /** Status PDV do servidor — só na edição. */
  pdvStatus?: UserPdvStatus;
};

/**
 * Settings de e-mails / “abrir PDV” legados ficam fora — a API ainda não
 * persiste esses campos. Credenciais de caixa (código + PIN) vivem em
 * `UserPdvSection`. Flag **Usuário vendedor** (`isSeller`) está na seção Geral.
 */
