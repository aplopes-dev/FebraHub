...
export async function listVariationsPaginated(
  ...
  const response = await comercioFetch<VariationListResponseDto>(
...
export async function listAllVariations(): Promise<Variation[]> {
  const response = await comercioFetch<{ data: VariationDto[] }>(
    "/v1/variations",
  );
...
export async function getVariationById(
  ...
  const response = await comercioFetch<VariationResponseDto>(
    `/v1/variations/${id}`,
  );
...
export async function uploadVariationOptionImage(
  ...
  const response = await comercioUpload<VariationResponseDto>(
...
export async function deleteVariation(id: string): Promise<void> {
  await comercioFetch<void>(`/v1/variations/${id}`, { method: "DELETE" });
}
