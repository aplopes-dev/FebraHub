 ...
 * cross-tenant) — por isso `X-Organization-Id` viaja igual ao `comercioFetch`,
...
export async function fiscalFetch<T>(
  ...
  const res = await fetchWithSession(`${FISCAL_PROXY}${path}`, {
    ...init,
    headers,
  });
 ...
 * apagaria o boundary e quebraria o multipart). Espelha `comercioUpload`.
 */
...
/**
 * Baixa um XML/PDF (DANFE/DANFSE) da fiscal-api e dispara o download direto
 * no navegador (spec erp/029, decisão do clarify — sem abrir aba
 * intermediária). Reusado pelos 3 pontos de UI (Vendas, Pedidos de venda,
 * Facilita NF-e) — nenhum deles reimplementa a mecânica de download.
 *
 * Não usa `fiscalFetch` (que sempre faz `JSON.parse` do corpo) — aqui o
 * corpo é o próprio arquivo.
 */
export async function downloadFiscalDocument(
  ...
  const res = await fetchWithSession(`${FISCAL_PROXY}${path}`, { headers });
  if (!res.ok) {
    const parsed = await extractErrorInfo(res);
    throw new FiscalApiError(res.status, parsed.message, parsed.code);
  }
  ...
  try {
    ...
    // Precisa estar no DOM para o `download` disparar de forma confiável
    // (achado react-review, spec erp/029) — alguns browsers ignoram `click()`
    // num `<a>` nunca anexado.
