const OAUTH_PENDING_KEY = 'citybox-comercio-oauth-pending';
const TAB_ID_KEY = 'citybox-comercio-tab-id';
...
/**
 * O pending é guardado por aba: duas abas iniciando login ao mesmo tempo
 * sobrescreveriam o `code_verifier` uma da outra e a segunda troca falharia.
 */
...
export function peekOAuthPending(expectedState: string): OAuthPending | null {
  ...
  try {
    ...
    // O `state` amarra o callback ao pedido que esta aba fez — é a defesa
    // contra CSRF no fluxo OAuth.
...
export async function beginOAuthAuthorization(
  ...
  // Sem default. Havia um fallback para o client do realm compartilhado aqui:
  // um app mal configurado pedia token silenciosamente para o client errado,
  // em vez de falhar (ADR C-17, bloco 6).
