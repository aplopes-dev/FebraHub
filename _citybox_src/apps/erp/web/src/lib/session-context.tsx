...
/** Jitter para várias abas não baterem no BFF no mesmo instante. */
...
export function SessionProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async (): Promise<Session | null> => {
    ...
    // Dedupe: o polling e um 401 podem pedir sincronização ao mesmo tempo.
  ...
  useEffect(() => {
    registerSessionBridge({ refresh, patchUser: () => undefined });
  ...
  return (
    <SessionContext.Provider
      value={{ status, session, loggingOut, refresh, logout }}
    >
      {children}
    </SessionContext.Provider>
  );
