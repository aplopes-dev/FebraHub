...
/**
 * Cópia local do antigo `@citybox/nest-common` (ADR C-17, bloco 5).
 *
 * Não há pacote compartilhado de autenticação: cada sistema mantém a sua cópia,
 * e a consistência entre elas é garantida pelo ADR C-17, não pelo compilador.
 *
 * Duas mudanças em relação ao original:
 * 1. `provisionMember()` perdeu `verticalRole` / `realmRole` — com um realm por
 *    sistema (ADR C-16) **estar no realm já é o gate de acesso**; as roles
 *    `vertical.imoveis.view` e `store_staff` deixaram de existir.
 * 2. A credencial é `imoveis-provisioning` (do realm `citybox-imoveis`), não o
 *    antigo `citybox-core-admin` global.
 */
export type KeycloakProvisioningConfig = {
  /** Ex.: http://127.0.0.1:8080/realms/citybox-imoveis */
  issuer: string;
...
/** Subset de `RequestInit` — evita depender de lib DOM. */
type AdminFetchInit = {
  method?: string;
  body?: string;
...
/**
 * Provisionamento de usuário no realm `citybox-imoveis`.
 *
 * ## O que NÃO faz
 *
 * Não persiste nada: quem grava o `TeamMember` é a própria imoveis-api, que
 * também é dona da decisão de rollback se a persistência falhar depois do
 * provisionamento.
 */
@Injectable()
export class KeycloakProvisioningService {
   ...
   *
   * Não concede role nenhuma: com realm por sistema, pertencer ao realm
   * `citybox-imoveis` já é a autorização de entrada; o que o membro pode fazer
   * dentro do Imóveis vem de `TeamMember.permissions` (CASL), não do token.
   */
