...
public typealias CityBoxAPIAPIChallengeHandler = ((URLSession, URLSessionTask, URLAuthenticationChallenge) -> (URLSession.AuthChallengeDisposition, URLCredential?))
...
private var challengeHandlerStore = SynchronizedDictionary<Int, CityBoxAPIAPIChallengeHandler>()
...
open class URLSessionRequestBuilder<T>: RequestBuilder<T> {
     ...
     */
    public var taskDidReceiveChallenge: CityBoxAPIAPIChallengeHandler?
    ...
    @discardableResult
    override open func execute(_ apiResponseQueue: DispatchQueue = CityBoxAPIAPI.apiResponseQueue, _ completion: @escaping (_ result: Swift.Result<Response<T>, ErrorResponse>) -> Void) -> RequestTask {
    ...
    open func buildHeaders() -> [String: String] {
        ...
        for (key, value) in CityBoxAPIAPI.customHeaders {
        ...
        case is URL.Type:
            do {
                ...
                if let headerFileName = getFileName(fromContentDisposition: httpResponse.allHeaderFields["Content-Disposition"] as? String) {
                    ...
                    requestPath = requestPath.appending("/tmp.CityBoxAPI.\(UUID().uuidString)")
                }
