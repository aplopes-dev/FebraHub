...
// Restaurar sessão (Splash → auto-login)
AuthAPI.getSession() { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Login com Google (A1)
AuthAPI.loginGoogle(loginGoogleRequest: loginGoogleRequest) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Logout
AuthAPI.logout(logoutRequest: logoutRequest) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Sincronizar onboarding (pós-login, A3)
AuthAPI.onboardingPostLogin(onboardingPostLoginRequest: onboardingPostLoginRequest) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Marcar onboarding visto (pré-login, A3)
AuthAPI.onboardingPreLogin(onboardingPreLoginRequest: onboardingPreLoginRequest) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Renovar access token
AuthAPI.refreshToken(logoutRequest: logoutRequest) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
