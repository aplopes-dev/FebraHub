...
const QUEUE = 'beautiful.store-setup';
const ROUTING_KEY = 'citybox.store.#';

const HANDLED_VERTICALS: readonly StorePlatformVertical[] = ['Beautiful'];
...
/**
 * Liga a beautiful-api ao ciclo de vida da loja no admin-api.
 *
 * | evento               | efeito local                                      |
 * |----------------------|---------------------------------------------------|
 * | `store.created`      | Org+Store+OWNER Keycloak → callback               |
  ...
  implements OnApplicationBootstrap, OnModuleDestroy
{
  ...
  constructor(
    ...
    private readonly seedFinancial: SeedFinancialDefaultsService,
    private readonly dedupe: EventDedupeService,
  ) {}

  async onApplicationBootstrap(): Promise<void> {
    if (process.env.BEAUTIFUL_WORKER_ENABLED === 'false') {
      this.logger.warn(
        'Consumidor store-setup desligado (BEAUTIFUL_WORKER_ENABLED=false)',
      );
      return;
    }
    ...
    if (!url) {
      this.logger.error(
        'RABBITMQ_URL ausente — a API sobe sem consumir eventos; lojas Beautiful criadas no admin não serão provisionadas.',
      );
      return;
    }
  ...
  private async provision(data: StorePlatformEventData): Promise<void> {
    try {
      ...
      await this.seedFinancial.execute(data.storeId);
