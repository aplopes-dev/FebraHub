...
export type KeycloakProvisioningConfig = {
  /** Realm **deste** sistema. Ex.: http://127.0.0.1:8080/realms/citybox-beautiful */
  issuer: string;
  /** Service account do próprio sistema: `beautiful-provisioning` (ADR C-16). */
  clientId: string;
  clientSecret: string;
};
...
/** Subset de `RequestInit` — evita depender de lib DOM neste app Nest/Node. */
type AdminFetchInit = {
  method?: string;
  body?: string;
...
/**
 * Provisionamento de usuário no Keycloak — cópia local do beautiful-api.
 *
 * Era `@citybox/nest-common`. Com um realm por sistema (ADR C-16) e sem pacote
 * compartilhado de autenticação (ADR C-17), cada app mantém a sua cópia: a
 * credencial usada aqui é o service account `beautiful-provisioning`, que tem
 * `manage-users` **apenas** no realm `citybox-beautiful`.
 *
 * ## Por que isto existe
 *
 * A criação do usuário no Keycloak e a criação do `Member` local são **dois
 * dados em dois lugares**, e nada os mantém sincronizados sozinho. Se um for
 * criado sem o outro, o usuário fica com acesso "fantasma" (identidade sem
 * `Member` — passa no AuthGuard e falha no StoreScopeGuard) ou "invisível"
 * (`Member` sem identidade — nunca consegue logar).
 *
 * ## O que NÃO faz
 *
 * Não persiste nada: quem grava o `Member` é o módulo `tenancy`, dono também da
 * decisão de rollback se a persistência falhar depois do provisionamento.
 *
 * ## O que saiu em relação ao `nest-common`
 *
 * `verticalRole` / `realmRole` (`vertical.beautiful.view`, `store_staff`) e todo
 * o maquinário de client roles (`ensureVerticalRole`, `deprovisionMember`,
 * `findBackofficeClient`): com realm próprio, **estar no realm já é o gate de
...
export class KeycloakProvisioningService {
  ...
  async findUserByUsernameOrEmail(
    ...
    const { body } = await this.admin<
      Array<{ id: string; username: string; email?: string | null }>
    >(`/users?exact=true&username=${query}`);
    if (body?.length) return toKeycloakUser(body[0]);

    const byEmail = await this.admin<
      Array<{ id: string; username: string; email?: string | null }>
    >(`/users?exact=true&email=${query}`);
    return byEmail.body?.length ? toKeycloakUser(byEmail.body[0]) : null;
  }

  findByEmail(email: string): Promise<KeycloakUser | null> {
    const normalized = email.trim().toLowerCase();
    if (!normalized) return Promise.resolve(null);
    return this.findUserByUsernameOrEmail(normalized);
  }
  ...
  /**
   * Cria (ou reaproveita) o usuário no realm do próprio sistema.
   * Ver a nota de sincronia identidade↔`Member` no topo da classe.
   */
  async provisionMember(
    ...
    if (fromLocation) return { keycloakSub: fromLocation, reused: false };
   ...
   *   Use `false` quando a senha é só exibida no app e a troca forçada fica a
   *   cargo do admin da plataforma.
   */
  ...
  private async patchUser(
    ...
    const { body: existing, status } = await this.admin<
  ...
  async deleteUser(keycloakSub: string): Promise<void> {
    const res = await this.admin(`/users/${keycloakSub}`, { method: 'DELETE' });
    if (res.status >= 400 && res.status !== 404) {
      throw new KeycloakProvisioningError(
        `Falha ao remover usuário ${keycloakSub}: ${res.status}`,
        res.status,
      );
    }
  }
...
function toKeycloakUser(raw: {
  id: string;
  username: string;
  email?: string | null;
}): KeycloakUser {
  return { id: raw.id, username: raw.username, email: raw.email ?? null };
}
