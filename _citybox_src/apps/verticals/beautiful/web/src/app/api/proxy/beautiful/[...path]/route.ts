...
/**
 * Rotas que não operam sobre uma loja específica — não exigem `X-Store-Id`.
 * `members/me` é a descoberta de acesso (circular se exigir loja).
 */
...
async function proxy(req: NextRequest, segments: string[]) {
  ...
  const target = `${beautifulApiBase()}/${path}${query ? `?${query}` : ''}`;
