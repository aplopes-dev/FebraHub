...
import type { ClientCategory } from '../services/client-categories-service';
import {
  useClientCategoriesQuery,
  useCreateClientCategoryMutation,
  useDeleteClientCategoryMutation,
  useUpdateClientCategoryMutation,
} from '../hooks/use-client-categories-queries';

export function SettingsClientCategoriesPage() {
  const canCreateCategory = useCan('create', 'Category');
  const canUpdateCategory = useCan('update', 'Category');
  const { data = [], isPending, isError } = useClientCategoriesQuery();
  const createMutation = useCreateClientCategoryMutation();
  const updateMutation = useUpdateClientCategoryMutation();
  const deleteMutation = useDeleteClientCategoryMutation();
  ...
  const openEdit = (category: ClientCategory) => {
    setEditing(category);
    setName(category.name);
    setColorId(normalizeCategoryHex(category.colorId));
    setNameError('');
    setFormOpen(true);
  };
  ...
  const handleSubmit = () => {
    ...
    if (editing) {
      updateMutation.mutate(
        { id: editing.id, name: trimmed, colorId: normalizeCategoryHex(colorId) },
        {
          onSuccess: () => {
            setFormOpen(false);
    ...
    createMutation.mutate(
      { name: trimmed, colorId: normalizeCategoryHex(colorId) },
      {
        onSuccess: () => {
          setFormOpen(false);
  ...
  const columns: DataTableColumn<ClientCategory>[] = [
    {
      ...
      render: (row) => (
        <Stack direction="row" spacing={1} sx={{ alignItems: 'center' }}>
          <CategoryColorBadge colorId={row.colorId} label={row.name} />
          {row.isProtected ? (
            <Chip label="Protegida" size="small" variant="outlined" />
          ) : null}
        </Stack>
      ),
    },
  ...
  return (
    <SettingsShell>
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            <FormField
              ...
              fullWidth
            />
            <CategoryColorField value={colorId} onChange={setColorId} disabled={busy} />
          </Stack>
        </DialogContent>
      ...
      <ConfirmationDialog
        ...
        description={
          deleteTarget
            ? `A categoria "${deleteTarget.name}" será removida. Clientes vinculados ficam sem categoria.`
            : undefined
        }
          ...
          deleteMutation.mutate(deleteTarget.id, {
            ...
            onError: () =>
              toast.error('Não foi possível excluir a categoria', {
                description: 'Categorias protegidas ou em uso não podem ser removidas.',
              }),
          });
        }}
      />
