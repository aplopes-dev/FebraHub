...
import { hasBackofficeAccess, resolveBackofficePermissions } from '@/lib/vertical-permissions';

/** Secret de dev do Keycloak local: `<clientId>-dev-secret` (ADR C-17). */
const DEV_SECRET_FALLBACK = 'beautiful-web-dev-secret';
...
/**
 * Issuer público (browser / OAuth redirect) — realm `citybox-beautiful`.
 * Sem default: um app mal configurado deve falhar, não apontar para o realm
 * errado (ADR C-16, invariante 1).
 */
...
/** Client web deste app: `beautiful-web`. Nunca compartilhado com outro sistema. */
...
const DEV_ORIGIN = 'http://127.0.0.1:3115';
/** Origem pública de produção (domínio do salão). */
const PROD_ORIGIN = 'https://salao.aplopes.com';

function allowedRedirectOrigins(): Set<string> {
  const origins = new Set([
    PROD_ORIGIN,
    'https://beautiful.citybox.com',
    'http://localhost:3115',
    'http://127.0.0.1:3115',
  ]);
...
/** Origem do app Beautiful para redirects OAuth/logout. */
 ...
 * Manter `revokeRefreshToken: false` no Keycloak até cache distribuído.
 */
 ...
 * Gate do backoffice.
 *
 * Com um realm por sistema (ADR C-16), **estar no realm `citybox-beautiful` já
 * é o gate** — não há mais `vertical.beautiful.view` para conferir. O que este
 * check garante é só que o token é decodificável e veio deste realm; o acesso a
 * uma loja específica continua no `StoreScopeGuard` da API.
 */
export function accessTokenGrantsBackoffice(accessToken: string): boolean {
  ...
  if (payload.iss !== keycloakIssuer() && payload.iss !== keycloakServerIssuer()) {
    return false;
  }
...
export function decodeJwtPerms(token: string): string[] {
  ...
  const client =
    (payload.resource_access as Record<string, { roles?: string[] }> | undefined)?.[
      process.env.NEXT_PUBLIC_KEYCLOAK_CLIENT ?? ''
    ]?.roles ?? [];
  return [...realm, ...client];
}
...
export function publicSessionFromTokens(
  ...
  return {
    ...
    permissions: resolveBackofficePermissions(decodeJwtPerms(accessToken)),
  };
}
...
export function beautifulApiBase(): string {
  return process.env.BEAUTIFUL_API_URL ?? 'http://127.0.0.1:3173/api';
}
