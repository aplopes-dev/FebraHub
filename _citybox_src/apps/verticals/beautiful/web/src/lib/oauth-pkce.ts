...
export async function beginOAuthAuthorization(redirectUri: string, force = false): Promise<string> {
  ...
  // Sem default. Havia `?? 'citybox-backoffice'`: um app mal configurado
  ...
  // Com realms separados, abrir o Beautiful logado no admin já pede login — são
  // sessões distintas. `prompt=login` segue útil para trocar de conta aqui.
