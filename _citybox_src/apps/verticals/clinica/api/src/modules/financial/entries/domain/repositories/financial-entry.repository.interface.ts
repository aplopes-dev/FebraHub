...
export type FinancialEntryPatientRef = {
  id: string;
  name: string;
  cpf: string | null;
};
...
export type FinancialEntryLoaded = {
  ...
  patient: FinancialEntryPatientRef | null;
};
...
/** Débitos do card Inadimplência (income de pacientes inadimplentes agora). */
...
export abstract class FinancialEntryRepository {
  ...
  /**
   * true se o lançamento é origem de um CommissionAccrual
   * ou despesa gerada por CommissionPayment.
   */
  abstract isLinkedToCommission(
    storeId: string,
    entryId: string,
  ): Promise<boolean>;
   ...
   * (inclusive civil dates), with patient name.
   */
  ...
  /**
   * Income entries with source=avulso_debit and dueDate in range
   * (inclusive civil dates), with patient name — used for sales mode.
   */
  abstract listAvulsoDebitsInDueDateRange(
    storeId: string,
    startIsoDate: string,
    endIsoDate: string,
  ): Promise<FinancialEntryLoaded[]>;
  ...
  /**
   * Inadimplência: incomes pending|received com paciente e dueDate no range,
   * apenas de pacientes que hoje têm overdue (pending + dueDate < todayKey).
   */
  ...
  /** Anos distintos de dueDate (income pending|received com paciente). */
