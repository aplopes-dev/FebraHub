...
function matchesCriteria(
  ...
  if (criteria.hasReceipt === true && !entry.receiptObjectKey) return false;
  if (criteria.hasReceipt === false && entry.receiptObjectKey) return false;
  if (
  ...
  if (criteria.patientId && entry.patientId !== criteria.patientId) {
    return false;
  }
...
function toLoaded(
  ...
  return {
    ...
    patient: entry.patientId
      ? {
          id: entry.patientId,
          name: patientNames?.get(entry.patientId) ?? 'Paciente',
          cpf: null,
        }
      : null,
  };
...
export class InMemoryFinancialEntryRepository extends FinancialEntryRepository {
  ...
  /** IDs de lançamentos ligados a comissão (para testes do guard). */
  private linkedToCommissionIds = new Set<string>();
  private readonly patientNames = new Map<string, string>();
  private readonly patientPhones = new Map<string, string>();
  ...
  seedPatientPhone(patientId: string, phone: string): void {
    this.patientPhones.set(patientId, phone);
  }
  ...
  markLinkedToCommission(entryId: string): void {
    this.linkedToCommissionIds.add(entryId);
  }
  ...
  async findById(
    ...
    return entry ? toLoaded(entry, this.patientNames) : null;
  }
  ...
  async findMany(
    ...
    const filtered = this.entries.filter(
      (entry) =>
        entry.storeId === storeId && matchesCriteria(entry, criteria),
    );
    return sortEntries(filtered, criteria)
      .slice(criteria.skip, criteria.skip + criteria.take)
      .map((entry) => toLoaded(entry, this.patientNames));
  }
  ...
  async computeStats(
    ...
    for (const entry of this.entries) {
      ...
      if (entry.type === 'income') {
        ...
        else if (entry.status === 'pending') incomeToReceive += entry.valueCents;
  ...
  async isLinkedToCommission(
    ...
    const exists = this.entries.some(
      (entry) => entry.id === entryId && entry.storeId === storeId,
    );
    return exists && this.linkedToCommissionIds.has(entryId);
  }
    ...
    return this.entries
      .filter(
        (entry) =>
          entry.storeId === storeId &&
          entry.type === 'income' &&
          entry.source === 'avulso_debit' &&
          matchesPeriod(entry.dueDate, startIsoDate, endIsoDate),
      )
      .map((entry) => toLoaded(entry, this.patientNames));
  }
  ...
  async listTicketMedioDayMetricsInRange(
    ...
    for (const entry of this.entries) {
      ...
      if (entry.type === 'income' && entry.status === 'received') {
        ...
        if (entry.patientId) bucket.patientIds.add(entry.patientId);
  ...
  async listInadimplenciaDebtsInRange(
    ...
    const delinquentPatientIds = new Set<string>();
    for (const entry of this.entries) {
      if (entry.storeId !== storeId) continue;
      if (entry.type !== 'income') continue;
      if (entry.status !== 'pending') continue;
      ...
      delinquentPatientIds.add(entry.patientId);
    }
    ...
    return this.entries
      .filter((entry) => {
        ...
        if (!delinquentPatientIds.has(entry.patientId)) return false;
      ...
      .map((entry) => ({
        ...
        patientName: this.patientNames.get(entry.patientId!) ?? 'Paciente',
        phone: this.patientPhones.get(entry.patientId!) ?? null,
      }));
  }
