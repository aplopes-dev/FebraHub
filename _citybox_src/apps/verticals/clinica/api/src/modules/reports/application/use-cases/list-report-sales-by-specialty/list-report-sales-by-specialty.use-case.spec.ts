import { ListReportSalesBySpecialtyUseCase } from './list-report-sales-by-specialty.use-case';
import { InMemoryReportSalesBySpecialtyRepository } from '../../../tests/in-memory-report-sales-by-specialty.repository';

const STORE_A = '11111111-1111-4111-8111-111111111111';
const STORE_B = '22222222-2222-4222-8222-222222222222';

describe('ListReportSalesBySpecialtyUseCase', () => {
  function createHarness() {
    const repo = new InMemoryReportSalesBySpecialtyRepository();
    const useCase = new ListReportSalesBySpecialtyUseCase(repo);
    return { repo, useCase };
  }
  ...
  it('lists approved budget items whose approvedAt is in range', async () => {
    ...
    expect(result.items[0]?.specialtyName).toBe('Ortodontia');
  ...
  it('falls back to budget date when approvedAt is null', async () => {
    ...
    expect(result.items[0]?.specialtyName).toBe('Não informado');
  });
