...
/**
 * Dedupe de eventos consumidos.
 *
 * O outbox do platform-api entrega **at-least-once**: crash entre o publish e a marcação
 * republica a linha quando o lease expira. Sem isto, um `store.created` reentregue
 * tentaria provisionar de novo.
 *
  ...
  async claim(
    ...
    try {
      ...
      if (isUniqueViolation(err)) {
        this.logger.log(`Evento ${eventId} (${type}) já processado — ignorando`);
        return false;
      }
      throw err;
    }
