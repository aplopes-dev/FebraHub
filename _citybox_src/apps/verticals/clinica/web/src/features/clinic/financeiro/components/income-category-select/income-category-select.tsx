...
type IncomeCategorySelectProps = {
...
export function IncomeCategorySelect({
  ...
  const { canCreateCategory } = useFinancialPermissions();
  const { data: categories = [], isLoading, isError } = useIncomeCategories();
  ...
  const handleCreate = async (input: { name: string; color: string }) => {
    try {
      ...
      toast.error(
        err instanceof ClinicaApiError
          ? err.message
