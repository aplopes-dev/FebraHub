...
} from "@/features/clinic/modules/patients/lib/patient-budget-form-utils";
...
import { usePayEntry } from "../hooks/use-pay-entry";
import type { FinancialEntry } from "../types";
import { ExpenseReceiptViewerSheet } from "./expense-receipt-viewer-sheet";

type PayExpenseSheetProps = {
  open: boolean;
...
export function PayExpenseSheet({
  open,
  onOpenChange,
  entry,
  viewMode = false,
  onAttachReceipt,
}: PayExpenseSheetProps) {
    ...
    try {
      await payEntry({
        ...
        data: {
          ...
          paidAt: toIsoDateOnly(values.receivedDate),
    ...
    <>
      <Sheet open={open} onOpenChange={onOpenChange}>
        ...
        >
          ...
          >
            <SheetTitle>
              {viewMode ? "Detalhes do pagamento" : "Registrar Pagamento"}
            </SheetTitle>
            <Button
              type="button"
              variant="ghost"
          ...
          <SheetFooter
            className={cn(
              CLINIC_SHEET_FOOTER_CLASS,
              viewMode &&
                (onAttachReceipt || (entry.hasReceipt && entry.receiptUrl)) &&
                "justify-between",
            )}
          >
            ...
            ) : viewMode && entry.hasReceipt && entry.receiptUrl ? (
              <Button
                type="button"
                variant="outline"
                className={CLINIC_SHEET_FOOTER_BUTTON_CLASS}
            ...
            <div className="flex gap-3">
              ...
              {!viewMode ? (
                <Button
                  type="button"
                  className={CLINIC_SHEET_FOOTER_BUTTON_CLASS}
                  disabled={isPending || cashRegisters.length === 0}
