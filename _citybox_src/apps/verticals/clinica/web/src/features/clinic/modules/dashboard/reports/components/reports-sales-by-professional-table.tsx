...
import { useReportSalesByProfessionalQuery } from '../hooks/use-report-sales-by-professional-query';
import { resolveReportBudgetPeriodRange } from '../lib/reports-period';
import type {
  ReportBudgetPeriodMode,
  ReportSalesByProfessionalRow,
...
type ReportsSalesByProfessionalTableProps = {
  budgetPeriodMode: ReportBudgetPeriodMode;
  budgetMonth: number;
  budgetYear: number;
};

export function ReportsSalesByProfessionalTable({
  budgetPeriodMode,
  budgetMonth,
  budgetYear,
}: ReportsSalesByProfessionalTableProps) {
  ...
  const { data, isLoading, isPending, isError, isFetching } =
    useReportSalesByProfessionalQuery({
  ...
  const columns = useMemo<ColumnDef<ReportSalesByProfessionalRow>[]>(
    () => [
      {
        id: 'professionalName',
  ...
  if (isError) {
    return (
      <p className="py-10 text-center text-sm text-destructive">
        Não foi possível carregar as vendas por profissional.
      </p>
    );
  }
  ...
  if (rows.length === 0) {
    return (
      <ReportsEmptyState
        title="Nenhuma venda por profissional no período."
  ...
  return (
    <div className={isFetching ? 'opacity-80' : undefined}>
      <ReportsDataTable
        ...
        pageSize={PER_PAGE}
        entityName="vendas"
        emptyMessage="Nenhuma venda por profissional no período."
