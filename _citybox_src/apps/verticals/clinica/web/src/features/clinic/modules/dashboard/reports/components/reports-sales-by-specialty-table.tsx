...
import { useReportSalesBySpecialtyQuery } from '../hooks/use-report-sales-by-specialty-query';
import { resolveReportBudgetPeriodRange } from '../lib/reports-period';
import type {
  ReportBudgetPeriodMode,
  ReportSalesBySpecialtyRow,
...
type ReportsSalesBySpecialtyTableProps = {
  budgetPeriodMode: ReportBudgetPeriodMode;
  budgetMonth: number;
  budgetYear: number;
};

export function ReportsSalesBySpecialtyTable({
  budgetPeriodMode,
  budgetMonth,
  budgetYear,
}: ReportsSalesBySpecialtyTableProps) {
  ...
  const { data, isLoading, isPending, isError, isFetching } =
    useReportSalesBySpecialtyQuery({
  ...
  const columns = useMemo<ColumnDef<ReportSalesBySpecialtyRow>[]>(
    () => [
      {
        id: 'specialtyName',
  ...
  if (isError) {
    return (
      <p className="py-10 text-center text-sm text-destructive">
        Não foi possível carregar as vendas por especialidade.
      </p>
    );
  }
  ...
  if (rows.length === 0) {
    return (
      <ReportsEmptyState
        title="Nenhuma venda por especialidade no período."
  ...
  return (
    <div className={isFetching ? 'opacity-80' : undefined}>
      <ReportsDataTable
        ...
        pageSize={PER_PAGE}
        entityName="vendas"
        emptyMessage="Nenhuma venda por especialidade no período."
