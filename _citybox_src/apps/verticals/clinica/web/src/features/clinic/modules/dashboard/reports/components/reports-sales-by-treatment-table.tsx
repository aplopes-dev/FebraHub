...
import { useReportSalesByTreatmentQuery } from '../hooks/use-report-sales-by-treatment-query';
import { resolveReportBudgetPeriodRange } from '../lib/reports-period';
import type {
  ReportBudgetPeriodMode,
  ReportSalesByTreatmentRow,
...
type ReportsSalesByTreatmentTableProps = {
  budgetPeriodMode: ReportBudgetPeriodMode;
  budgetMonth: number;
  budgetYear: number;
};

export function ReportsSalesByTreatmentTable({
  budgetPeriodMode,
  budgetMonth,
  budgetYear,
}: ReportsSalesByTreatmentTableProps) {
  ...
  const { data, isLoading, isPending, isError, isFetching } =
    useReportSalesByTreatmentQuery({
  ...
  const columns = useMemo<ColumnDef<ReportSalesByTreatmentRow>[]>(
    () => [
      {
        id: 'treatmentName',
  ...
  if (isError) {
    return (
      <p className="py-10 text-center text-sm text-destructive">
        Não foi possível carregar as vendas por procedimento.
      </p>
    );
  }
  ...
  if (rows.length === 0) {
    return (
      <ReportsEmptyState
        title="Nenhuma venda por procedimento no período."
  ...
  return (
    <div className={isFetching ? 'opacity-80' : undefined}>
      <ReportsDataTable
        ...
        pageSize={PER_PAGE}
        entityName="vendas"
        emptyMessage="Nenhuma venda por procedimento no período."
