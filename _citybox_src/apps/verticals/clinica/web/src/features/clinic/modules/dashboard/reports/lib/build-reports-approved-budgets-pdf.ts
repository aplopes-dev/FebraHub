...
export type BuildReportsApprovedBudgetsPdfInput = {
...
export function buildReportsApprovedBudgetsPdfFileName(
  generatedAt = new Date(),
): string {
  const stamp = formatLocalDateString(generatedAt);
  return `relatorio-orcamentos-aprovados-${slugifyPatientPdfFileNamePart(stamp)}.pdf`;
}
...
/**
 * PDF do relatório Orçamentos aprovados com header da clínica
 * e tabela das colunas do relatório.
 */
export async function buildReportsApprovedBudgetsPdf(
  input: BuildReportsApprovedBudgetsPdfInput,
): Promise<Blob> {
