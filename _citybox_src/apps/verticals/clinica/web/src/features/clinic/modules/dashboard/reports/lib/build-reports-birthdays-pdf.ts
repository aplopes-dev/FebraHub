...
const TABLE_COLUMNS = [
  ...
  { header: 'Data do aniversário', width: 42 },
  { header: 'Celular', width: 45 },
] as const;

export type BuildReportsBirthdaysPdfInput = {
  rows: readonly ReportBirthdayRow[];
  periodLabel: string;
  clinic: PatientPdfClinicInfo;
  generatedAt?: Date;
};

export function buildReportsBirthdaysPdfFileName(
  generatedAt = new Date(),
): string {
  const stamp = formatLocalDateString(generatedAt);
  return `relatorio-aniversariantes-${slugifyPatientPdfFileNamePart(stamp)}.pdf`;
}
...
/**
 * PDF do relatório Aniversariantes com header da clínica (logo + dados da empresa)
 * e tabela Paciente / Telefone / Data do aniversário / Celular.
 */
export async function buildReportsBirthdaysPdf(
  input: BuildReportsBirthdaysPdfInput,
): Promise<Blob> {
  ...
  if (input.rows.length === 0) {
    ...
    for (const row of input.rows) {
      ...
      const values = [
        ...
        formatLocalDateBr(row.birthDate),
