...
const TABLE_COLUMNS = [
  ...
  { header: 'Valor', width: 36 },
  { header: 'Porcentagem', width: 36 },
] as const;

export type BuildReportsExpensesByCategoryPdfInput = {
  rows: readonly ReportExpensesByCategoryRow[];
  periodLabel: string;
  clinic: PatientPdfClinicInfo;
  generatedAt?: Date;
};

export function buildReportsExpensesByCategoryPdfFileName(
  generatedAt = new Date(),
): string {
  const stamp = formatLocalDateString(generatedAt);
  return `relatorio-despesas-por-categoria-${slugifyPatientPdfFileNamePart(stamp)}.pdf`;
}
...
/**
 * PDF do relatório Despesas por categoria com header da clínica
 * e tabela das colunas do relatório.
 */
export async function buildReportsExpensesByCategoryPdf(
  input: BuildReportsExpensesByCategoryPdfInput,
): Promise<Blob> {
  ...
  drawPatientPdfMetaRows(
    ...
    [
      ...
      `Total: ${input.rows.length} categoria(s)`,
    ],
  ...
  if (input.rows.length === 0) {
    ...
    doc.text(
      'Nenhuma despesa por categoria no período selecionado.',
      PATIENT_PDF_PAGE_MARGIN_X,
      writer.cursorY + 5,
    );
    writer.cursorY += 10;
