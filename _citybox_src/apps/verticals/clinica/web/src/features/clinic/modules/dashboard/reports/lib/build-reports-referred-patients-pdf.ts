...
const TABLE_COLUMNS = [
  { header: 'Paciente indicado', width: 40 },
  { header: 'Quem indicou?', width: 36 },
  { header: 'Data indicação', width: 28 },
  { header: '1ª consulta', width: 28 },
  { header: 'Orç. aprovados', width: 30 },
] as const;

export type BuildReportsReferredPatientsPdfInput = {
  rows: readonly ReportReferredPatientRow[];
  periodLabel: string;
  clinic: PatientPdfClinicInfo;
  generatedAt?: Date;
};

export function buildReportsReferredPatientsPdfFileName(
  generatedAt = new Date(),
): string {
  const stamp = formatLocalDateString(generatedAt);
  return `relatorio-pacientes-indicados-${slugifyPatientPdfFileNamePart(stamp)}.pdf`;
}
...
/**
 * PDF do relatório Pacientes indicados com header da clínica
 * e tabela das colunas do relatório.
 */
export async function buildReportsReferredPatientsPdf(
  input: BuildReportsReferredPatientsPdfInput,
): Promise<Blob> {
  ...
  if (input.rows.length === 0) {
    ...
    for (const row of input.rows) {
      ...
      const values = [
        truncate(row.referredPatientName || '—', 24),
        truncate(row.referredBy || '—', 22),
        formatLocalDateBr(row.referralDate),
        row.firstAppointmentDate
          ? formatLocalDateBr(row.firstAppointmentDate)
          : '—',
