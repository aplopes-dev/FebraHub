...
export type BuildReportsRejectedBudgetsPdfInput = {
...
export function buildReportsRejectedBudgetsPdfFileName(
  generatedAt = new Date(),
): string {
  const stamp = formatLocalDateString(generatedAt);
  return `relatorio-orcamentos-reprovados-${slugifyPatientPdfFileNamePart(stamp)}.pdf`;
}
...
/**
 * PDF do relatório Orçamentos reprovados com header da clínica
 * e tabela das colunas do relatório.
 */
export async function buildReportsRejectedBudgetsPdf(
  input: BuildReportsRejectedBudgetsPdfInput,
): Promise<Blob> {
  ...
  if (input.rows.length === 0) {
    ...
    doc.text(
      'Nenhum orçamento reprovado no período selecionado.',
      PATIENT_PDF_PAGE_MARGIN_X,
      writer.cursorY + 5,
    );
    writer.cursorY += 10;
