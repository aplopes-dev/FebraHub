...
export type BuildReportsSalesByProfessionalPdfInput = {
  rows: readonly ReportSalesByProfessionalRow[];
  periodLabel: string;
  clinic: PatientPdfClinicInfo;
  generatedAt?: Date;
};

export function buildReportsSalesByProfessionalPdfFileName(
  generatedAt = new Date(),
): string {
  const stamp = formatLocalDateString(generatedAt);
  return `relatorio-vendas-por-profissional-${slugifyPatientPdfFileNamePart(stamp)}.pdf`;
}
...
/**
 * PDF do relatório Vendas por profissional com header da clínica
 * e tabela das colunas do relatório.
 */
export async function buildReportsSalesByProfessionalPdf(
  input: BuildReportsSalesByProfessionalPdfInput,
): Promise<Blob> {
  ...
  if (input.rows.length === 0) {
    ...
    doc.text(
      'Nenhuma venda por profissional no período selecionado.',
      PATIENT_PDF_PAGE_MARGIN_X,
      writer.cursorY + 5,
    );
    writer.cursorY += 10;
    ...
    for (const row of input.rows) {
      ...
      const values = [
        truncate(row.professionalName || '—', 22),
