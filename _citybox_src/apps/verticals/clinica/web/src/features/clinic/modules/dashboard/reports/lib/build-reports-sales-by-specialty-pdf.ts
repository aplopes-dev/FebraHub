...
export type BuildReportsSalesBySpecialtyPdfInput = {
  rows: readonly ReportSalesBySpecialtyRow[];
  periodLabel: string;
  clinic: PatientPdfClinicInfo;
  generatedAt?: Date;
};

export function buildReportsSalesBySpecialtyPdfFileName(
  generatedAt = new Date(),
): string {
  const stamp = formatLocalDateString(generatedAt);
  return `relatorio-vendas-por-especialidade-${slugifyPatientPdfFileNamePart(stamp)}.pdf`;
}
...
/**
 * PDF do relatório Vendas por especialidades com header da clínica
 * e tabela das colunas do relatório.
 */
export async function buildReportsSalesBySpecialtyPdf(
  input: BuildReportsSalesBySpecialtyPdfInput,
): Promise<Blob> {
  ...
  drawPatientPdfMetaRows(
    ...
    { title: 'Vendas por especialidades' },
  );
  ...
  if (input.rows.length === 0) {
    ...
    doc.text(
      'Nenhuma venda por especialidade no período selecionado.',
      PATIENT_PDF_PAGE_MARGIN_X,
      writer.cursorY + 5,
    );
    writer.cursorY += 10;
