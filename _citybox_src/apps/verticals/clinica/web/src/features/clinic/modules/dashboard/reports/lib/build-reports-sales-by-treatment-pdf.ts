...
export type BuildReportsSalesByTreatmentPdfInput = {
  rows: readonly ReportSalesByTreatmentRow[];
  periodLabel: string;
  clinic: PatientPdfClinicInfo;
  generatedAt?: Date;
};

export function buildReportsSalesByTreatmentPdfFileName(
  generatedAt = new Date(),
): string {
  const stamp = formatLocalDateString(generatedAt);
  return `relatorio-vendas-por-procedimento-${slugifyPatientPdfFileNamePart(stamp)}.pdf`;
}
...
/**
 * PDF do relatório Vendas por procedimentos com header da clínica
 * e tabela das colunas do relatório.
 */
export async function buildReportsSalesByTreatmentPdf(
  input: BuildReportsSalesByTreatmentPdfInput,
): Promise<Blob> {
  ...
  drawPatientPdfMetaRows(
    ...
    { title: 'Vendas por procedimentos' },
  );
  ...
  if (input.rows.length === 0) {
    ...
    doc.text(
      'Nenhuma venda por procedimento no período selecionado.',
      PATIENT_PDF_PAGE_MARGIN_X,
      writer.cursorY + 5,
    );
    writer.cursorY += 10;
