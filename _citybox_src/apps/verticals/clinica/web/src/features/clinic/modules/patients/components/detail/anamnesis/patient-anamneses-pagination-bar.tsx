...
export const PATIENT_ANAMNESIS_PAGE_SIZE_OPTIONS = [5, 10, 20, 50] as const;

export type PatientAnamnesisPageSize = (typeof PATIENT_ANAMNESIS_PAGE_SIZE_OPTIONS)[number];

type PatientAnamnesesPaginationBarProps = {
  page: number;
...
export function PatientAnamnesesPaginationBar({
  page,
  pageSize,
  total,
  totalPages,
  onPageChange,
  onPageSizeChange,
}: PatientAnamnesesPaginationBarProps) {
      ...
      <div className="flex items-center justify-between gap-3 sm:contents">
        <p className="justify-self-start whitespace-nowrap text-sm text-muted-foreground">
          ...
          <span className="hidden xl:inline">
            Exibindo {start}–{end} de {total} anamnese{total === 1 ? '' : 's'}
          </span>
        </p>
        ...
        <div className="flex shrink-0 items-center gap-2 sm:order-last sm:justify-self-end">
          ...
          <Select
            ...
            onValueChange={(value) => onPageSizeChange(Number(value) as PatientAnamnesisPageSize)}
          >
            <SelectTrigger className="h-9 w-20 bg-card">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {PATIENT_ANAMNESIS_PAGE_SIZE_OPTIONS.map((option) => (
