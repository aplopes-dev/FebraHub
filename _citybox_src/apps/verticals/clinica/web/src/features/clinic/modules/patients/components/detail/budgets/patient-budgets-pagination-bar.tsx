...
export const PATIENT_BUDGET_PAGE_SIZE_OPTIONS = [5, 10, 20, 50] as const;

export type PatientBudgetPageSize = (typeof PATIENT_BUDGET_PAGE_SIZE_OPTIONS)[number];

type PatientBudgetsPaginationBarProps = {
  page: number;
...
export function PatientBudgetsPaginationBar({
  page,
  pageSize,
  total,
  totalPages,
  onPageChange,
  onPageSizeChange,
}: PatientBudgetsPaginationBarProps) {
    ...
    <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-2 border-t border-border/60 pt-4">
      <div className="flex flex-nowrap items-center gap-1.5 sm:gap-2">
        <p className="shrink-0 whitespace-nowrap text-sm text-muted-foreground">
          Exibindo {start}–{end} de {total} orçamento{total === 1 ? '' : 's'}
        </p>
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        <Select
          ...
          onValueChange={(value) => onPageSizeChange(Number(value) as PatientBudgetPageSize)}
        >
          <SelectTrigger className="h-9 w-20 bg-card">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            {PATIENT_BUDGET_PAGE_SIZE_OPTIONS.map((option) => (
