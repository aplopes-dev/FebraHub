...
export const PATIENT_FINANCIAL_PAGE_SIZE_OPTIONS = [5, 10, 20, 50] as const;

export type PatientFinancialPageSize = (typeof PATIENT_FINANCIAL_PAGE_SIZE_OPTIONS)[number];

type PatientFinancialPaginationBarProps = {
  page: number;
...
export function PatientFinancialPaginationBar({
  page,
  pageSize,
  total,
  totalPages,
  onPageChange,
  onPageSizeChange,
}: PatientFinancialPaginationBarProps) {
      ...
      <div className="flex items-center justify-between gap-3 sm:contents">
        <p className="justify-self-start whitespace-nowrap text-sm text-muted-foreground">
          ...
          <span className="hidden xl:inline">
            Exibindo {start}–{end} de {total} lançamento{total === 1 ? '' : 's'}
          </span>
        </p>
        ...
        <div className="flex shrink-0 items-center gap-2 sm:order-last sm:justify-self-end">
          ...
          <Select
            ...
            onValueChange={(value) => onPageSizeChange(Number(value) as PatientFinancialPageSize)}
          >
            <SelectTrigger className="h-9 w-20 bg-card">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {PATIENT_FINANCIAL_PAGE_SIZE_OPTIONS.map((option) => (
