...
export const PATIENT_TREATMENT_PAGE_SIZE_OPTIONS = [5, 10, 20, 50] as const;

export type PatientTreatmentPageSize = (typeof PATIENT_TREATMENT_PAGE_SIZE_OPTIONS)[number];

type PatientTreatmentsPaginationBarProps = {
  page: number;
  pageSize: PatientTreatmentPageSize;
  total: number;
  totalPages: number;
  onPageChange: (page: number) => void;
  onPageSizeChange: (pageSize: PatientTreatmentPageSize) => void;
};
...
function getVisiblePages(page: number, totalPages: number): (number | 'ellipsis')[] {
  ...
  // Janela compacta — evita 1 2 … N empurrar "Por página" para a linha de baixo no tablet.
  if (page <= 2) {
...
export function PatientTreatmentsPaginationBar({
  page,
  pageSize,
  total,
  totalPages,
  onPageChange,
  onPageSizeChange,
}: PatientTreatmentsPaginationBarProps) {
    ...
    <div className="flex flex-col gap-3 border-t border-border/60 pt-4 sm:grid sm:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] sm:items-center sm:gap-4">
      <div className="flex items-center justify-between gap-3 sm:contents">
        <p className="justify-self-start whitespace-nowrap text-sm text-muted-foreground">
          ...
          <span className="hidden xl:inline">
            Exibindo {start}–{end} de {total} procedimento{total === 1 ? '' : 's'}
          </span>
        </p>
        ...
        <div className="flex shrink-0 items-center gap-2 sm:order-last sm:justify-self-end">
          ...
          <Select
            ...
            onValueChange={(value) => onPageSizeChange(Number(value) as PatientTreatmentPageSize)}
          >
            <SelectTrigger className="h-9 w-20 bg-card">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {PATIENT_TREATMENT_PAGE_SIZE_OPTIONS.map((option) => (
      ...
      <Pagination className="mx-0 w-auto justify-center sm:justify-self-center">
        <PaginationContent className="flex-nowrap gap-0.5">
          ...
          {/* Tablet: indicador compacto. Desktop: números de página. */}
