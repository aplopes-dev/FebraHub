...
import { WHATSAPP_VARIABLE_CATALOG } from '../data/whatsapp-variable-catalog';
import {
  WHATSAPP_VARIABLE_DRAG_MIME,
  type WhatsappVariable,
} from '../types/whatsapp';

type WhatsappVariablesSidebarProps = {
  className?: string;
  onSelectVariable?: (variable: WhatsappVariable) => void;
};
...
function DraggableVariableChip({
  ...
  variable: WhatsappVariable;
  onSelect?: (variable: WhatsappVariable) => void;
}) {
  const handleDragStart = (event: React.DragEvent<HTMLButtonElement>) => {
    event.dataTransfer.setData(WHATSAPP_VARIABLE_DRAG_MIME, variable.token);
...
export function WhatsappVariablesSidebar({
  className,
  onSelectVariable,
}: WhatsappVariablesSidebarProps) {
  return (
    <aside
      className={cn(
    ...
    >
      <div className="border-b border-border/60 px-4 py-4">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Clique ou arraste para o texto da mensagem
        </p>
      </div>

      <div className="min-h-0 flex-1 overflow-y-auto bg-muted/50 px-3 py-4">
        <div className="space-y-2">
          {WHATSAPP_VARIABLE_CATALOG.map((variable) => (
