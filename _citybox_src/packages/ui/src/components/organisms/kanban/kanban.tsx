...
> & {
  ...
  /** Classes no container com scroll vertical da coluna. */
  viewportClassName?: string;
  /** Style no container (ex. `maxHeight` para N cards visíveis). */
...
export function KanbanCards<T extends KanbanItem = KanbanItem>({
  ...
  // Native overflow — Radix ScrollArea + maxHeight não cria scroll confiável
  // (viewport `h-full` sem altura definida no root).
  return (
    <div
      className={cn(
...
> = {
  ...
  className?: string;
  /**
   * Id estável do `DndContext` (SSR). Sem isso o `@dnd-kit` gera
   * `DndDescribedBy-N` via contador de módulo, que diverge entre server e
   * client e dispara warning de hidratação em `aria-describedby`.
   * Default: `useId()` do React (seguro entre SSR e client).
   */
  id?: string;
  /**
   * `scroll` (padrão): colunas em faixa com scroll horizontal (`w-max`).
   * `fill`: colunas dividem a largura disponível — sem scroll H (ex.: 5 status
   * fixos no board de leads). Combine com `KanbanBoard` `flex-1 min-w-0 basis-0`.
   */
  layout?: KanbanLayout;
...
>({
  ...
  // Portal do DragOverlay só após mount — `typeof window` no render causa
  // mismatch SSR (sem portal) vs client (com portal).
  ...
  return (
    <KanbanContext.Provider value={{ data, activeCardId }}>
      ...
      >
        ...
        >
          <ScrollAreaPrimitive.Viewport className="size-full [&>div]:!flex [&>div]:h-full">
            <div
              className={cn(
                "flex gap-3 pb-3",
                layout === "fill"
