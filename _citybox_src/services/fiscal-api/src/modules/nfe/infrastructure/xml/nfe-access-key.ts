...
export type BuildAccessKeyInput = {
  ...
  series: string;
  number: string;
  /// Tipo de emissão — "1" normal (único suportado no v1; contingência é
  /// evolução futura).
  tpEmis: string;
  /// Código numérico aleatório (8 dígitos) — gerado pelo chamador para poder
  /// ser determinístico em testes; em produção usar um gerador aleatório real.
  cNF: string;
};
