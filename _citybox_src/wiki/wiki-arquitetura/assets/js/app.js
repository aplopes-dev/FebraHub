(function () {
  ...
  var FEEDBACK_KEY = 'wiki_arquitetura_feedback';

  const NAV_GROUPS = [
    {
      id: 'intro',
      label: 'Introdução',
      items: ['visao-geral', 'equivalencia-stack']
    },
    {
      id: 'diagnostico',
      label: 'Diagnóstico',
      items: ['matriz-maturidade']
    },
    {
      id: 'arquitetura-alvo',
      label: 'Arquitetura-alvo',
      items: ['arquitetura-alvo', 'comunicacao-eventos', 'dados-distribuidos', 'saga-checkout', 'resiliencia-observabilidade', 'seguranca-devops']
    },
    {
      id: 'evolucao',
      label: 'Evolução',
  ...
  function updateProgress() {
    var all = getAllFeedback();
    var total = WIKI.sections.length;
    var done  = Object.keys(all).filter(function(id){ return all[id].status === 'approved'; }).length;
  ...
  function renderFeedbackWidget(sectionId) {
    ...
    return '<div class="feedback-widget" id="feedbackWidget">' +
      ...
      '<div class="feedback-desc">Leia o conteúdo acima e indique se aprova o desenho proposto ou se deseja solicitar ajustes.</div>' +
  ...
  function initExport() {
    document.addEventListener('click', function (e) {
      if (e.target && e.target.id === 'exportFeedbackBtn') {
        var all  = getAllFeedback();
        var text = 'RELATÓRIO DE APROVAÇÃO — Wiki Arquitetura Citybox\n';
