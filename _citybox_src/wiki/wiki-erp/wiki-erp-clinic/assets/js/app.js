(function () {
  ...
  var FEEDBACK_KEY = 'wiki_clinic_feedback';

  const NAV_GROUPS = [
    {
      id: 'intro',
      label: 'Introdução',
      items: ['visao-geral-clinic', 'benchmark-clinic', 'tipos-estabelecimentos', 'onboarding-multiclinica']
    },
    {
      id: 'agenda',
      label: 'Agenda e Recepção',
      items: ['agenda-agendamento', 'encaixe-compromissos-retorno', 'recepcao-sala-espera']
    },
    {
      id: 'prontuario',
      label: 'Prontuário Clínico',
      items: ['prontuario-eletronico', 'anamnese-formularios', 'odontograma-especialidades', 'orcamentos-tratamentos', 'documentos-contratos', 'arquivos-paciente']
    },
    {
      id: 'financeiro',
      label: 'Financeiro',
      items: ['financeiro-caixa', 'faturamento-convenios']
    },
    {
      id: 'operacoes',
      label: 'Operações',
      items: ['estoque-suprimentos', 'equipe-rh', 'loja-assinatura-eletronica']
    },
    {
      id: 'crescimento',
      label: 'CRM e Crescimento',
      items: ['crm-funil-vendas', 'captacao-leads']
    },
    {
      id: 'inteligencia',
      label: 'Inteligência e Config',
      items: ['relatorios-bi', 'configuracoes-parametros']
    },
    {
      id: 'paciente',
      label: 'Portal do Paciente',
      items: ['portal-paciente']
    },
    {
      id: 'design',
      label: 'Interfaces e UX',
      items: ['interfaces-ux']
    },
    {
      id: 'tecnico',
      label: 'Técnico e Conformidade',
      items: ['lgpd-conformidade', 'arquitetura-tecnica']
    },
    {
      id: 'evolucao',
      label: 'Evolução',
      items: ['roadmap-clinic', 'como-aprovar-clinic']
    }
  ];
  ...
  function updateProgress() {
    var all = getAllFeedback();
    var excludeIds = ['como-aprovar-clinic'];
  ...
  function renderFeedbackWidget(sectionId) {
    if (sectionId === 'como-aprovar-clinic') return '';
  ...
  function renderSection(section) {
    ...
    if (section.id === 'como-aprovar-clinic') { populateApprovalSummary(); }
  }
  ...
  function populateApprovalSummary() {
    ...
    WIKI.sections.forEach(function (s) {
      if (s.id === 'como-aprovar-clinic') return;
  ...
  function initExport() {
    document.addEventListener('click', function (e) {
      if (e.target && e.target.id === 'exportFeedbackBtn') {
        var all  = getAllFeedback();
        var text = 'RELATÓRIO DE APROVAÇÃO — Wiki Clinic Citybox\n';
