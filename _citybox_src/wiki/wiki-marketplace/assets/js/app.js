(function () {
  ...
  var FEEDBACK_KEY = 'wiki_marketplace_feedback';

  const NAV_GROUPS = [
    {
      id: 'intro',
      label: 'Introdução',
      items: ['visao-geral-marketplace', 'benchmark-marketplace', 'jornada-consumidor']
    },
    {
      id: 'descoberta-busca',
      label: 'Descoberta e Busca',
      items: ['home-descoberta', 'busca-typesense', 'loja-vitrine-oferta']
    },
    {
      id: 'carrinho-checkout',
      label: 'Carrinho e Checkout',
      items: ['carrinho-multiloja', 'checkout-orquestracao', 'pagamento-split']
    },
    {
      id: 'pedido-posvenda',
      label: 'Pedido e Pós-venda',
      items: ['acompanhamento-pedido', 'pos-venda-suporte', 'notificacoes-engajamento', 'fluxos-navegacao']
    },
    {
      id: 'engajamento',
      label: 'Engajamento',
      items: ['fidelidade-promocoes']
    },
    {
      id: 'plataforma',
      label: 'Plataforma Multi-vertical',
      items: ['multivertical-catalogo', 'tenancy-municipal']
    },
    {
      id: 'apps-arq',
      label: 'Apps e Arquitetura',
      items: ['app-nativo-pwa', 'arquitetura-marketplace', 'modelo-dados-projecoes', 'backend-mock', 'contrato-bff-api']
    },
    {
      id: 'design',
      label: 'Design e Mockups',
      items: ['mockups-app', 'prototipos-html']
    },
    {
      id: 'evolucao',
      label: 'Evolução',
      items: ['estado-implementacao', 'roadmap-marketplace', 'como-aprovar-marketplace']
    }
  ];
  ...
  function updateProgress() {
    var all = getAllFeedback();
    var excludeIds = ['como-aprovar-marketplace'];
  ...
  function renderFeedbackWidget(sectionId) {
    if (sectionId === 'como-aprovar-marketplace') return '';
  ...
  function renderSection(section) {
    ...
    if (section.id === 'como-aprovar-marketplace') { populateApprovalSummary(); }
  }
  ...
  function populateApprovalSummary() {
    ...
    WIKI.sections.forEach(function (s) {
      if (s.id === 'como-aprovar-marketplace') return;
  ...
  function initExport() {
    document.addEventListener('click', function (e) {
      if (e.target && e.target.id === 'exportFeedbackBtn') {
        var all  = getAllFeedback();
        var text = 'RELATÓRIO DE APROVAÇÃO — Wiki Marketplace Citybox\n';
