...
import { NeuralDetail } from './NeuralDetail';
...
const hoverKind = (id: string): string => {
  if (id.startsWith('emp:')) return 'agente de IA';
  if (id.startsWith('person:')) return 'funcionário';
  if (id.startsWith('task:')) return 'função';
  if (id.startsWith('tool:')) return 'ferramenta';
...
export function NeuralGraph({
  ...
  return (
    ...
    >
      <svg viewBox={`0 0 ${NEURAL_W} ${NEURAL_H}`} className="block w-full" role="img" aria-label="Vista neural do organograma">
        <defs>
