  ...
  if (collapsed) {
    return (
      <button
        ...
        title="Mostrar diretório"
        aria-label="Mostrar diretório"
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between border-b border-os-border px-3 py-2">
        <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-os-dim">Diretório</span>
        {onToggleCollapse && (
          <button
            type="button"
            onClick={onToggleCollapse}
            title="Ocultar diretório"
            aria-label="Ocultar diretório"
            ...
            <div key={g.kind} className={gi > 0 ? 'mt-2 border-t border-os-border pt-0.5' : undefined}>
              ...
              {g.rows.map((r) => (
                <button
                  ...
                  title={`Mostrar ${r.label} no grafo`}
                >
                  ...
                  />
                  <span className="min-w-0 flex-1 truncate text-[13px] leading-snug text-os-text">
                    {r.label}
                  </span>
