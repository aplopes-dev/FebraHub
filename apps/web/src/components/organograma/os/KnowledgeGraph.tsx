...
const CAT: Record<KGNodeKind, { color: string; Icon: LucideIcon; label: string; r: number }> = {
  self: { color: 'var(--text)', Icon: Sparkles, label: 'Núcleo', r: 18 },
...
export function KnowledgeGraph({
  ...
  const compactLegend = (
    <div className="flex items-center gap-3 rounded-sm-t border border-os-border-strong bg-os-bg/85 px-2.5 py-1.5 backdrop-blur">
      {(
        [
          { label: 'Núcleo', color: HUB_COLOR, Icon: CAT.self.Icon },
          { label: 'Funcionário', color: CAT.person.color, Icon: CAT.person.Icon },
          { label: 'Agente de IA', color: CAT.employee.color, Icon: CAT.employee.Icon },
          { label: 'Ferramenta', color: CAT.tool.color, Icon: CAT.tool.Icon },
          { label: 'Função', color: CAT.task.color, Icon: CAT.task.Icon },
        ] as const
  ...
  const vaultSearchInput = coreExpanded ? (
    <input
      ...
      title="Aperte / para buscar no cofre"
      spellCheck={false}
  ...
  const taskCard = selectedTask ? (
    <SopTaskDetailCard
      ...
      assigneeKindLabel={selectedTask.assigneeKind === 'person' ? 'funcionário' : 'agente de IA'}
  ...
  const graphInner = (
    <>
      ...
      <svg
        ...
        role="img"
        aria-label="Grafo do organograma"
        onClick={clearAll}
      >
        ...
        {nodes.map((n) => {
          ...
          if (n.kind === 'self' && memoryOn) {
            return (
              ...
              >
                <title>Núcleo — clique para abrir</title>
                {memoryCoreInner}
        ...
        <div className="relative h-[680px] min-w-0 flex-1 overflow-hidden rounded-lg-t border border-os-border bg-os-surface">
          ...
          {focusSet && !coreExpanded && (
            <>
              <button
                onClick={() => stepDept(-1)}
                aria-label="Girar para o setor anterior"
                title="Setor anterior (←)"
              ...
              <button
                onClick={() => stepDept(1)}
                aria-label="Girar para o próximo setor"
                title="Próximo setor (→)"
          ...
          {(agentCard || toolWiki || taskCard || humanCard || memoryCard) && (
            <div className="kg-panel absolute right-3 top-[46px] z-10 flex max-h-[calc(100%-58px)] w-[300px] flex-col overflow-hidden rounded-lg-t border border-os-border-strong bg-os-bg">
              <button
                onClick={clearDetail}
                aria-label={`Voltar para o setor ${focusedTeam?.label ?? 'grafo'}`}
              ...
              >
                ...
                <span className="truncate">
                  Voltar · <span style={{ color: focusedTeam?.color ?? 'var(--text)' }}>{focusedTeam?.label ?? 'grafo'}</span>
                </span>
              </button>
              {toolWiki ? (
        ...
        <aside className={`flex max-h-[560px] shrink-0 flex-col gap-3.5 rounded-lg-t border border-os-border bg-os-surface p-3 lg:h-[680px] lg:max-h-none ${directoryCollapsed ? 'w-auto lg:w-16' : 'w-full lg:w-72'}`}>
          <div className={directoryCollapsed ? 'hidden' : undefined}>
            ...
            <div className="mb-3 flex flex-col gap-1">
              {(
                [
                  ['Entidade', ENTITY_LENSES],
                  ['Área', FUNCTION_LENSES],
                  ['Ação', ACTION_LENSES],
                ] as [string, Lens[]][]
              ).filter(([, lenses]) => lenses.length > 0).map(([groupLabel, lenses]) => (
                <select
                  key={groupLabel}
                  value={lenses.some((l) => l.id === lensId) ? lensId! : ''}
                  onChange={(e) => setLensId(e.target.value || null)}
                  aria-label={`Lente de ${groupLabel}`}
                ...
                >
                  <option value="">{groupLabel} · todas</option>
                  {lenses.map((l) => (
            ...
            <div className="mb-1.5 font-mono text-[9px] uppercase tracking-[0.16em] text-os-dim">Legenda</div>
