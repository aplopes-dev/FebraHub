...
export function KnowledgeGraphFullscreen({
  ...
  const overlay = (
    ...
    // FebraHub: +`fh-os` — o portal monta em document.body, FORA do wrapper
    // que carrega as variáveis de tema do OS; sem a classe aqui, o fullscreen
    // renderiza sem nenhum token (único desvio da cópia literal).
    <div className="fh-os fixed inset-0 z-[140] flex bg-os-bg">
      ...
      <div className="relative min-w-0 flex-1 overflow-hidden bg-os-surface">
        ...
        {!coreOpen && (
          <div className="absolute left-5 top-5 z-20 flex items-center gap-2.5 rounded-sm-t border border-os-border-strong bg-os-bg/85 px-2.5 py-1.5 backdrop-blur">
            <div className="flex flex-col">
              ...
              >
                {currentDept?.name ?? 'Escolha um setor'}
              </span>
              <span className="font-mono text-[8.5px] uppercase tracking-[0.14em] text-os-dim">
                {idx >= 0 ? `${idx + 1} / ${deptList.length}` : `${deptList.length} setores`}
              </span>
            </div>
        ...
        {/* FebraHub: o diretório sai de cena enquanto um card de detalhe está
            aberto — antes o card (z-30) simplesmente cobria o diretório. */}
        {directorySlot && !hasDetail && (
      ...
      {hasDetail && (
        <aside className="absolute right-0 top-0 z-30 flex h-full w-[300px] flex-col border-l border-os-border-strong bg-os-bg shadow-lg max-[820px]:inset-x-0 max-[820px]:bottom-0 max-[820px]:top-auto max-[820px]:max-h-[62vh] max-[820px]:w-full max-[820px]:rounded-t-lg-t max-[820px]:border-l-0 max-[820px]:border-t">
          {/* the trail: node → pillar (this) → home. Same affordance inline. */}
          <button
            onClick={onBack}
            aria-label={`Voltar para o setor ${currentDept?.name ?? 'grafo'}`}
          ...
          >
            ...
            <span className="truncate">
              Voltar · <span style={currentDept ? { color: currentDept.color } : undefined}>{currentDept?.name ?? 'grafo'}</span>
            </span>
          </button>
          {toolWiki ? (
