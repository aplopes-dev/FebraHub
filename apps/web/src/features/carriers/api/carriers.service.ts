...
export async function listCarriers(
  ...
  const response = await apiFetch<CarrierListResponseDto>(
...
export async function getCarrierById(id: string): Promise<Carrier> {
  const response = await apiFetch<CarrierResponseDto>(
    `/v1/carriers/${id}`,
  );
...
export async function createCarrier(
  ...
  const response = await apiFetch<CarrierResponseDto>("/v1/carriers", {
    method: "POST",
...
export async function deleteCarrier(id: string): Promise<void> {
  await apiFetch<void>(`/v1/carriers/${id}`, { method: "DELETE" });
}
