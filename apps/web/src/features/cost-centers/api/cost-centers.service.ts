...
export async function listCostCenters(
  ...
  const response = await apiFetch<CostCenterListResponseDto>(
...
export async function getCostCenterById(id: string): Promise<CostCenter> {
  const response = await apiFetch<CostCenterResponseDto>(
    `/v1/cost-centers/${id}`,
  );
...
export async function deleteCostCenter(id: string): Promise<void> {
  await apiFetch<void>(`/v1/cost-centers/${id}`, { method: "DELETE" });
}
