...
export async function listFinancialEntriesApi(
  ...
  const res = await apiFetch<FinancialEntryListResponseDto>(
...
export async function findFinancialEntryByIdApi(
  ...
  const res = await apiFetch<{ data: FinancialEntryDetailDto }>(
...
export async function deleteFinancialEntryApi(id: string): Promise<void> {
  await apiFetch<void>(`/v1/financial-entries/${id}`, {
    method: "DELETE",
  });
}
...
export async function uploadFinancialEntryAttachmentApi(
  ...
  const res = await apiUpload<{ data: FinancialEntryAttachmentDto }>(
...
export function financialEntryAttachmentUrl(
  ...
  return `${API_PROXY}/v1/financial-entries/${financialEntryId}/attachments/${attachmentId}${query ? `?${query}` : ""}`;
}
