...
export async function listMovementCategoriesApi(
  ...
  const response = await apiFetch<MovementCategoryListResponseDto>(
...
export async function getMovementCategoryByIdApi(
  ...
  const response = await apiFetch<MovementCategoryResponseDto>(
...
export async function deleteMovementCategoryApi(id: string): Promise<void> {
  await apiFetch<void>(`/v1/movement-categories/${id}`, {
    method: "DELETE",
  });
}
