...
export async function getCustomerFiscalInfoApi(
  ...
  const res = await apiFetch<CustomerDetailRawDto>(`/v1/customers/${id}`);
  const data = res.data;
  return {
...
export async function listNfseIssuancesApi(): Promise<NfseIssuanceDto[]> {
  const res = await apiFetch<NfseIssuanceListResponseDto>(BASE);
  return res.data;
}
...
export async function issueNfseApi(
  ...
  const res = await apiFetch<NfseIssuanceResponseDto>(BASE, {
    method: "POST",
