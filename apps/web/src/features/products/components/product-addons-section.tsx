...
import { Button, Checkbox, NumberSpinner } from "@/ui";
...
export function ProductAddonsSection({
  ...
  const selectedAddonIds = useMemo(
    () => value.items.map((item) => item.addonProductId).filter(Boolean),
    [value.items],
  );
  ...
  function clampQuantity(next: number, fallback: number): number {
  ...
  return (
    <Box component="section" sx={productFormSectionGridSx}>
      <Box component="header" sx={productFormSectionHeaderSx}>
        ...
        >
          Vincule produtos já cadastrados (bacon extra, cheddar) que o cliente
          pode acrescentar a este item, com quantidade mínima e máxima.
        </Typography>
      </Box>
      <Box
        sx={{
        ...
        <Stack spacing={1}>
          ...
          {productsQuery.isError ? (
            <Typography variant="body2" color="error">
              Não foi possível carregar os produtos disponíveis para adicional.
            </Typography>
          ) : null}
          {!productsQuery.isError &&
          ...
          addonOptions.length === 0 ? (
            <Typography variant="body2" sx={{ color: "text.secondary" }}>
              Cadastre o adicional como produto em{" "}
              <Typography
                component={Link}
              ...
              (ex.: Bacon Extra) e volte aqui para vinculá-lo.
            </Typography>
          ) : null}
        </Stack>
      </Box>
