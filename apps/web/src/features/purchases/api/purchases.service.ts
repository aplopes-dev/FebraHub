...
export async function listPurchasesApi(
  ...
  const response = await apiFetch<PurchaseListResponseDto>(
...
export async function getPurchaseByIdApi(id: string): Promise<PurchaseDetail> {
  const response = await apiFetch<PurchaseDetailResponseDto>(
    `/v1/purchases/${id}`,
  );
...
export async function createPurchaseApi(
  ...
  await apiFetch<PurchaseSingleResponseDto>("/v1/purchases", {
    method: "POST",
...
export async function updatePurchaseApi(
  ...
  await apiFetch<PurchaseSingleResponseDto>(`/v1/purchases/${id}`, {
    method: "PUT",
    body: JSON.stringify(payload),
  });
}
...
export async function deletePurchaseApi(id: string): Promise<void> {
  await apiFetch<void>(`/v1/purchases/${id}`, { method: "DELETE" });
}

export async function restorePurchaseApi(id: string): Promise<void> {
  await apiFetch<PurchaseSingleResponseDto>(`/v1/purchases/${id}/restore`, {
    method: "POST",
  });
}
