...
export async function listStocksApi(
  ...
  const response = await apiFetch<StockListResponseDto>(
...
export async function getStockByIdApi(id: string): Promise<Stock> {
  const response = await apiFetch<StockResponseDto>(`/v1/stocks/${id}`);
...
export async function createStockApi(
  ...
  const response = await apiFetch<StockResponseDto>("/v1/stocks", {
    method: "POST",
...
export async function deleteStockApi(id: string): Promise<void> {
  await apiFetch<void>(`/v1/stocks/${id}`, { method: "DELETE" });
}
