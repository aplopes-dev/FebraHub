...
export async function listSuppliers(
  ...
  const response = await apiFetch<SupplierListResponseDto>(
...
export async function getSupplierById(id: string): Promise<Supplier> {
  const response = await apiFetch<SupplierResponseDto>(
    `/v1/suppliers/${id}`,
  );
...
export async function createSupplier(
  ...
  const response = await apiFetch<SupplierResponseDto>("/v1/suppliers", {
    method: "POST",
...
export async function deleteSupplier(id: string): Promise<void> {
  await apiFetch<void>(`/v1/suppliers/${id}`, { method: "DELETE" });
}
