...
const ORGANIZATION_KEY = "app.active-org";
const BRANCH_KEY = "app.active-branch";
...
export function OrganizationProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        // Uma empresa só: entra direto. Várias sem escolha salva: assume a
        // primeira — sem tela de seleção, deixar vazio travaria o app inteiro.
        const resolved =
          savedStillValid && saved ? saved : (list[0]?.id ?? "");
  ...
  // Unidades acompanham a organização ativa e o reload do tenancy.
  // `reload()` só incrementa `reloadToken`; sem ele neste efeito, criar/editar
  // filial atualizava a lista da tela mas o select do header ficava velho.
  useEffect(() => {
    let cancelled = false;
    ...
    void (async () => {
      try {
        ...
        // que o header lê como "todas as unidades".
        ...
        // Reload falhou: mantém a lista atual. No primeiro load ela já está vazia.
      } finally {
