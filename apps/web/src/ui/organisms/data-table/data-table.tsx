...
export type DataTableProps<T> = {
  ...
  sx?: BoxProps["sx"];
  /**
   * A rolagem fica no `ScrollArea` da página — a tabela cresce com o conteúdo
   * e não cria scroll interno no corpo.
   */
  pageScroll?: boolean;
...
/** Cabeçalho: fundo `muted` (`Neutral Lv1` do design) · h-11 · px-4 · uppercase. */
const headerCellSx = {
  bgcolor: "muted.main",
  fontWeight: 600,
  fontSize: "0.75rem",
 ...
 * (mesmo padrão do DataTable shadcn + ScrollArea do API do backend).
 */
        ...
        rows.map((row) => {
          ...
          return (
            <TableRow
              ...
              onClick={
                !isLinkRow && onRowClick ? () => onRowClick(row) : undefined
              }
              sx={
      ...
      sx={[
        {
          ...
          flex: pageScroll ? "none" : 1,
          minHeight: pageScroll ? "auto" : 0,
          gap: 2,
          width: "100%",
        },
